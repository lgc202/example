...
// 思路：快慢双指针
func removeElement27_1(nums []int, val int) int {
	slow := -1 // slow 指向已处理区域的最后一个元素
	fast := 0
	for fast < len(nums) {
		if nums[fast] != val {
			slow++
...
func removeElement27_2(nums []int, val int) int {
	slow := 0 // slow 指向已处理区域的下一个元素
	fast := 0
	for fast < len(nums) {
		if nums[fast] != val {
			nums[slow] = nums[fast]
			slow++ // 此时 slow 指向的是已处理区域的下一个元素
		}
